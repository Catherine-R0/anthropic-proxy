...
// Patterns that must NEVER appear in client-facing output
const FORBIDDEN_PATTERNS = [
  ...
  { pattern: /\bBig Five\b/,               category: "INTERNAL_LEAK",    description: "Technical model name: Big Five (use behavioral patterns instead)" },
  { pattern: /Matrix of Destiny/i,         category: "INTERNAL_LEAK",    description: "System name: Matrix of Destiny (use symbolic reflection layer)" },
  { pattern: /Матрица Судьбы/i,            category: "INTERNAL_LEAK",    description: "System name: Матрица Судьбы (use symbolic reflection layer)" },
  ...
  { pattern: /\bдиагноз\b/i,              category: "SAFETY",           description: "Medical term: диагноз" },
  { pattern: /\bdiagnosis\b/i,            category: "SAFETY",           description: "Medical term: diagnosis" },
...
// Required sections by language — at least one of these section titles must be present
const REQUIRED_SECTION_PATTERNS = {
  ru: [
    ...
    { key: "disclaimer",        pattern: /не является.*диагнозом|приглашени.*саморефлексии|не претендует/i },
  ],
  en: [
    ...
    { key: "disclaimer",        pattern: /not a.*diagnosis|invitation.*reflection|does not claim/i },
  ],
};

// Fallback patterns for other languages (check presence of section H2s)
